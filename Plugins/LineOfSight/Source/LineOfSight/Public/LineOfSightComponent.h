...
public:
	...
	UPROPERTY(EditAnywhere, Category = "LineOfSight Component")
		bool IgnoreOwnerActorInTraceLine;

	/* Calls Events: Hit Start and Hit End. Similar to Event On Component Begin Overlap. */
	UPROPERTY(EditAnywhere, Category = "LineOfSight Component")
		bool BeginAndEndOverlapEvent;

	/* Used for optimization. Use false if you only need 1 arch instead of Radius 1 = 0 and DirectionAngle 1 = 0. */
	UPROPERTY(EditAnywhere, Category = "LineOfSight Component")
		bool OnlyOneArc;
	...
	/**
	 * Opt-in physical collision generated from the visible procedural section.
	 * Disabled by default to preserve the plugin's original trace-only behavior.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "LineOfSight Component|Collision")
	bool bEnableDynamicMeshCollision;

	/**
	 * Half thickness of the simple collision prisms generated from each visible triangle.
	 * A non-zero volume is required because planar complex meshes cannot reliably own overlap state.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "LineOfSight Component|Collision",
		meta = (ClampMin = "0.1", UIMin = "0.1"))
	float DynamicMeshCollisionHalfThickness;

	/**
	 * Enables or disables physical collision on the generated section.
	 * Collision profile, enabled mode, responses and overlap generation remain consumer-owned.
	 */
	UFUNCTION(BlueprintCallable, Category = "LineOfSight Component|Collision")
	void SetDynamicMeshCollisionEnabled(bool bEnabled);

	UFUNCTION(BlueprintPure, Category = "LineOfSight Component|Collision")
	bool IsDynamicMeshCollisionEnabled() const { return bEnableDynamicMeshCollision; }

	/**
	 * Performs one immediate trace, mesh, collision and overlap refresh.
	 * This is intended for deterministic setup barriers and never replaces physical overlap authority.
	 */
	UFUNCTION(BlueprintCallable, Category = "LineOfSight Component")
	bool RefreshLineTraceAndMesh();
	...
	UFUNCTION(BlueprintCallable, Category = "LineOfSight Component")
		void SetIgnoreSelfLineTrace(const bool NewValueIgnoreSelf);
...
private:
	...
	void RefreshDynamicMeshOverlaps();
	void SetDynamicMeshCollisionPaused(bool bPaused);
