...
ULineOfSightComponent::ULineOfSightComponent(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	bEnableDynamicMeshCollision = false;
	DynamicMeshCollisionHalfThickness = 5.0f;
...
{
	if (bEnableDynamicMeshCollision == bEnabled)
	{
		return;
	}
	if (!bEnabled && bDynamicMeshCollisionPaused)
	{
		SetCollisionEnabled(CollisionModeBeforeDynamicPause);
		bDynamicMeshCollisionPaused = false;
	}

	bEnableDynamicMeshCollision = bEnabled;
	// Physical overlap ownership uses the exact union of simple triangular prisms below.
	// A planar complex-as-simple body is queryable by traces but does not reliably generate
	// persistent component overlap state.
	bUseComplexAsSimpleCollision = false;
	bTraceComplexOnMove = false;

	if (bBuildMeshActive && GetNumSections() > 0)
	{
	...
	{
		ClearDynamicMeshCollision();
	}
	UpdateOverlaps();
}

bool ULineOfSightComponent::RefreshLineTraceAndMesh()
{
	if (!StartLineTraceActive)
	{
		LINEOFSIGHT_LOG_WARNING(
			TEXT("LineOfSight component '%s' cannot refresh before StartLineTrace."),
			*GetNameSafe(this));
		return false;
	}
...
void ULineOfSightComponent::StartBuildMesh()
{
	...
	{
		LINEOFSIGHT_LOG_ERROR(
			TEXT("LineOfSight component '%s': StartBuildMesh can only be called after StartLineTrace."),
			*GetNameSafe(this));
		return;
	}
	...
	if (bEnableDynamicMeshCollision)
	{
		// StartLineTrace initializes the vertex buffer with zeroes. Cooking that placeholder
		// produces an invalid body and a later section update cannot reliably recover physical
		// overlaps in the same frame. Populate the real shape before the first collision cook.
		if (GeometryType == ETypeArc::Arc_VectorLenght)
		{
			BuildArcVectorLenght();
		}
	...
	{
		RebuildDynamicMeshCollision();
		SetDynamicMeshCollisionPaused(false);
	}
}
...
void ULineOfSightComponent::StartLineTrace(ETraceTypeQuery TraceChannel, int32 NumberOfLines)
{
	...
	if (IgnoreOwnerActorInTraceLine)
	{
...
TArray<FHitResult> ULineOfSightComponent::StartOAEArc(ETraceTypeQuery TraceChannel, float InRadius1, float InRadius2, float InAngle1, float InAngle2, int32 NumberOfLines)
{
	...
	for (int i = 0; i <= NumberOfLines; ++i)
	{
		...
		#if WITH_EDITORONLY_DATA
		if (DebugAOE && IsLineOfSightGlobalDebugEnabled())
		{
...
void ULineOfSightComponent::SetPauseTrace(const bool Pause, const bool RunEndOverlap, const bool EmptyOverlapArray, const bool ChangeVisibility)
{
	if (StartLineTraceActive)
	{
		...
		SetDynamicMeshCollisionPaused(PauseLineTrace || PauseBuildMesh);
	}
}
...
void ULineOfSightComponent::ZStartCloneTo(ULineOfSightComponent* OtherLineOfSightComponent)
{
	if (IsValid(OtherLineOfSightComponent))
	{
		...
		LineOfSightComponentForClone->CreateMeshSection_LinearColor(
			0,
			PointArray1,
			Triangle,
			NormalVertex,
...
{
	if (StartLineTraceActive || bBuildMeshActive || GetNumSections() > 0)
	{
		StopLineTrace();
	}
	Super::BeginDestroy();
...
void ULineOfSightComponent::RebuildDynamicMeshCollision()
{
	if (!bEnableDynamicMeshCollision
		|| !bBuildMeshActive
		|| DynamicMeshCollisionHalfThickness <= 0.0f)
	{
		ClearDynamicMeshCollision();
		return;
	}

	TArray<TArray<FVector>> ConvexPrisms;
	ConvexPrisms.Reserve(Triangle.Num() / 3);
	const FVector ThicknessOffset(
		0.0f,
		0.0f,
		DynamicMeshCollisionHalfThickness);
	for (int32 TriangleIndex = 0;
		TriangleIndex + 2 < Triangle.Num();
		TriangleIndex += 3)
	{
		const int32 Index0 = Triangle[TriangleIndex];
		const int32 Index1 = Triangle[TriangleIndex + 1];
		const int32 Index2 = Triangle[TriangleIndex + 2];
		if (!PointArray1.IsValidIndex(Index0)
			|| !PointArray1.IsValidIndex(Index1)
			|| !PointArray1.IsValidIndex(Index2))
		{
			continue;
		}

		const FVector& Vertex0 = PointArray1[Index0];
		const FVector& Vertex1 = PointArray1[Index1];
		const FVector& Vertex2 = PointArray1[Index2];
		if (FVector::CrossProduct(
				Vertex1 - Vertex0,
				Vertex2 - Vertex0).SizeSquared() <= UE_SMALL_NUMBER)
		{
			continue;
		}

		TArray<FVector>& Prism = ConvexPrisms.AddDefaulted_GetRef();
		Prism.Reserve(6);
		Prism.Add(Vertex0 - ThicknessOffset);
		Prism.Add(Vertex1 - ThicknessOffset);
		Prism.Add(Vertex2 - ThicknessOffset);
		Prism.Add(Vertex0 + ThicknessOffset);
		Prism.Add(Vertex1 + ThicknessOffset);
		Prism.Add(Vertex2 + ThicknessOffset);
	}

	SetCollisionConvexMeshes(ConvexPrisms);
	UpdateOverlaps();
	RefreshDynamicMeshOverlaps();
}

void ULineOfSightComponent::ClearDynamicMeshCollision()
{
	ClearComponentOverlaps(true, false);
	const TArray<TArray<FVector>> EmptyConvexMeshes;
	SetCollisionConvexMeshes(EmptyConvexMeshes);
	UpdateOverlaps();
}

void ULineOfSightComponent::RefreshDynamicMeshOverlaps()
{
	if (!bEnableDynamicMeshCollision
	...
	TArray<FOverlapResult> QueryResults;
	...
	TMap<TWeakObjectPtr<UPrimitiveComponent>, int32> CurrentOverlaps;
	for (const FOverlapResult& Result : QueryResults)
	{
		UPrimitiveComponent* OtherComponent = Result.Component.Get();
		if (!IsValid(OtherComponent)
			|| OtherComponent == this
			|| OtherComponent->GetOwner() == GetOwner()
			|| !OtherComponent->GetGenerateOverlapEvents()
			|| GetCollisionResponseToComponent(OtherComponent) != ECR_Overlap)
		{
			continue;
		}
		CurrentOverlaps.FindOrAdd(OtherComponent) = Result.GetItemIndex();
	}

	TArray<UPrimitiveComponent*> PreviousOverlaps;
	GetOverlappingComponents(PreviousOverlaps);
	for (UPrimitiveComponent* PreviousComponent : PreviousOverlaps)
	{
		if (IsValid(PreviousComponent)
			&& PreviousComponent->GetOwner() != GetOwner()
			&& !CurrentOverlaps.Contains(PreviousComponent))
		{
			EndComponentOverlap(
				FOverlapInfo(PreviousComponent),
				true,
				false);
		}
	}

	for (const TPair<TWeakObjectPtr<UPrimitiveComponent>, int32>& Pair :
		CurrentOverlaps)
	{
		UPrimitiveComponent* OtherComponent = Pair.Key.Get();
		if (IsValid(OtherComponent)
			&& !IsOverlappingComponent(OtherComponent))
		{
			BeginComponentOverlap(
				FOverlapInfo(OtherComponent, Pair.Value),
				true);
		}
	}
}

void ULineOfSightComponent::SetDynamicMeshCollisionPaused(const bool bPaused)
{
	if (!bEnableDynamicMeshCollision
		|| bDynamicMeshCollisionPaused == bPaused)
	{
		return;
	}
	...
	if (bPaused)
	{
		CollisionModeBeforeDynamicPause = GetCollisionEnabled();
		bDynamicMeshCollisionPaused = true;
		ClearDynamicMeshCollision();
...
void ULineOfSightComponent::BuildArcVectorLenght()
{
	...
	const float IncrementAngle1 = (Angle1 * 2) / Number_Of_Lines;		// This can be transferred to .h file, then the calculation of Increment DirectionAngle 1 should be in StartLineTrace(). If the angle changes, then recalculate the value in the StartInterpAngle() function.
	...
	for (int i = 0; i <= Number_Of_Lines; ++i)
	{
		...
		#if WITH_EDITORONLY_DATA
		if (Debug && IsLineOfSightGlobalDebugEnabled())
		{
			FVector HitLocation = (hit) ? HitResult.Last().Location : directionRotate2;
	...
	if (BeginAndEndOverlapEvent)
	{
		for (int32 i = WasStartOverlapArray.Num() - 1; i >= 0; --i)
		{
			if (AllHitActor.Contains(WasStartOverlapArray[i].GetActor()) == false)
			{
				...
				WasStartOverlapArray.RemoveAt(i, 1, EAllowShrinking::No);
			}
		}
	}
...
void ULineOfSightComponent::SetIgnoreSelfLineTrace(const bool NewValueIgnoreSelf)
{
	TraceParamsGlobal.ClearIgnoredSourceObjects();
	TraceParamsGlobal.AddIgnoredActor(GetOwner());
	IgnoreOwnerActorInTraceLine = NewValueIgnoreSelf;
}
