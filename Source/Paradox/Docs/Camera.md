...
All three actions trigger while paused. The existing click, touch, and Enter rewind mappings are
...
zoom and does not create a follow camera.
...
The maximum compatible orthographic width is recalculated from the actual viewport aspect ratio.
Pan, zoom, and recenter all use this same containment calculation. If a runtime aspect change makes
the configured minimum impossible, containment wins: the effective width is reduced temporarily
and one warning is emitted until the aspect becomes compatible again.

Camera updates use bounded real frame delta (`FApp::GetDeltaTime`) rather than world delta. Pan,
zoom, and recenter therefore stay frame-rate independent during Tactical Pause and time dilation.
